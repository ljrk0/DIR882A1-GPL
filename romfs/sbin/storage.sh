#!/bin/sh
#
# $Id: //WIFI_SOC/MP/SDK_5_0_0_0/RT288x_SDK/source/user/rt2880_app/scripts/storage.sh#25 $
#
# usage: storage.sh
#
#
...
setSmb()
{
	...
	if [ -e "$PART1" ]; then
		...
		DEF_SHARE=`nvram_get 2860 "smb_default_share"`
		if [ "$DEF_SHARE" == "" ]; then
			DEF_SHARE="$PART1/public"
		fi

		echo "samba_add_dir.sh Public "$DEF_SHARE" "$allusers""
		samba_add_dir.sh Public "$DEF_SHARE" "$allusers"
		sleep 1
		...
		if [ "$CONFIG_ARCH_MT7621" == "y" -a "$CONFIG_SAMBA_3_0_2" == "y" ]; then
			if [ -f "/proc/sys/net/ipv4/tcp_tso_win_divisor" ]; then
				echo 20 > /proc/sys/net/ipv4/tcp_tso_win_divisor
			fi
		fi
...
case $1 in
	...
	"samba")
		...
		if [ "$smbenabled" == "1" ]; then
			setSmb
			if [ -e "$PART1" ]; then
				boost_mode=`nvram_get 2860 boost_mode`
				if [ "$boost_mode" == "auto" -o "$boost_mode" == "storage" -o "$boost_mode" == "" ]; then
					smp.sh storage
				elif [ "$boost_mode" == "wifi" ]; then
					smp.sh wifi
				else
					smp.sh wifi
				fi
