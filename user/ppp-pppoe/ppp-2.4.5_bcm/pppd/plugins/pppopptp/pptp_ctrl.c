 ...
 *
 * $Id: //WIFI_SOC/MP/SDK_5_0_0_0/RT288x_SDK/source/user/ppp-2.4.2/pppd/plugins/pppopptp/pptp_ctrl.c#1 $
 */
...
int pptp_reboot_sent_clear(struct in_addr inetaddr, unsigned int call_id)
{
	...
	if (call_id) {
		...
		close_inetsock(conn->inet_sock, inetaddr); 
		free(conn);
	}
    return 0;
}
    ...
    switch (ntoh16(header->ctrl_type)) {
        ...
        case PPTP_OUT_CALL_RPLY:
        {
            ...
            if (packet->result_code != 1) {
                ...
                snprintf(cmd, sizeof(cmd), "echo %hu > /tmp/pptp_last_tunnel_id", call->peer_call_id);
                system(cmd);
                pptp_reset_timer();
