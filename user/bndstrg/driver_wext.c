				...
				u8 action)
{
	...
	struct driver_wext_data *drv_wext_data = \
					(struct driver_wext_data *)drv_data;	
	struct bndstrg_msg msg;
	memcpy(msg.nvram_entry.Addr, entry->Addr, MAC_ADDR_LEN);
	memcpy(msg.Addr, entry->Addr, MAC_ADDR_LEN);
	msg.nvram_entry.Band = entry->Band;
	msg.nvram_entry.Manipulable = entry->Manipulable ;
	msg.nvram_entry.Nss = entry->Nss ;
	msg.nvram_entry.PhyMode = entry->PhyMode;
	msg.Action = action;
	ret = driver_wext_set_oid(
				drv_wext_data,
				ifname,
