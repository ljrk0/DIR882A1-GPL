...
int cli_loop(struct cli_def *cli, int sockfd)
{
    ...
    while (1)
    {
        ...
        else if (cli->state == STATE_ENABLE_PASSWORD)
        {
            ...
            {
            	cli_error(cli, "\n\n Access enable_password :%s, cmd :%s", cli->enable_password,cmd);
...
int cli_loop_cli(struct cli_def *cli, int rsockfd, int sockfd)
{
    unsigned char c;
    int n, l, oldl = 0, is_telnet_option = 0, skip = 0, esc = 0;
    int cursor = 0, insertmode = 1;
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            if ((sr = select(rsockfd + 1, &r, NULL, NULL, &tm)) < 0)
            {
                /* select error */
                if (errno == EINTR)
                    continue;
