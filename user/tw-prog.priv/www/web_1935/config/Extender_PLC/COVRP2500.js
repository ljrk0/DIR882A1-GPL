//custom, these settings will override common settings 
 function DeviceInfo() {
this.createdDec="This Deviceinfo was created for COVRP2500, at Thu Jul 26 2018 10:47:36 GMT+0800 (CST)";
this.featureWPS=true;
this.featureSystemLog=true;
this.featureRebootSchedule=false;
this.featureCloseAdminHTTPS=false;
this.featureAdminstrationSettingHNAP=true;
this.featureBundled=false;
this.featureStatisticsWifi=false;
this.featureExtender=true;
this.featureSchedule=true;
this.ParentalControlWiFiOnly=false;
this.featureCloseCaptcha=true;
this.featureAddHNAP1Cgi=false;
this.featureSmartConnect=false;
this.featureChannel=false;
this.featureCovrWIFI=true;
this.featurePLCQoS=true;
this.featureQoSMgmtType=false;
this.featureQoSSpeed=false;
this.featureQosManageByApp=false;
this.okcount=20;
this.wizardcount=40;
this.rebootcount=120;
this.featureTalkTalkTr069SpecialOf1=false;
this.featureSystemLogEmailSettings=true;
this.featureNewTime=true;
this.featureAutoUpgrade =true;
}