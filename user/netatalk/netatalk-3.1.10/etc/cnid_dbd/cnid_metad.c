...
//added by hibachi 20170321
#define T_PATH_CNID_DBD	"/sbin/cnid_dbd"
#ifndef WEXITSTATUS
...
static int uid_from_name(const char *name)
{
    struct passwd *pwd;
...
//    char  *dbdpn = _PATH_CNID_DBD;
    ...
    while (1) {
        ...
        if (len[2]) {
            ...
            uidFlag = uid_from_name(username);
			if(uidFlag<0)
			{
				goto loop_end;
			}
...
//strcpy(vol->v_dbpath,"/mnt/SanDisk_Extreme_01291/Backups.backupdb/");
