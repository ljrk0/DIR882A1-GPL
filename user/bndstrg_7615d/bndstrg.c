...
void bndstrg_update_entry(struct bndstrg *bndstrg, struct bndstrg_msg *msg)
{
	...
	if(entry->band && (entry->band != msg->Band) && (msg->bConnStatus == FALSE))
	{
		DBGPRINT(DEBUG_OFF, "%s band mismatch\n", __FUNCTION__);
		return;
	}
...
#ifdef BND_STRG_QA
		BND_STRG_PRINTQAMSG(table, entry,
			(" %02x:%02x:%02x:%02x:%02x:%02x [%s client], bndstrg_matched_rule: %s Band:%s\n"),
			PRINT_MAC(entry->Addr),(IS_BND_STRG_DUAL_BAND_CLIENT(entry->Control_Flags)? "DUAL_BAND":"SINGLE_BAND"),
			((entry->matched_rule_id > -1)? bndstrg_matched_rule[entry->matched_rule_id] : "Unknown"),Cli_Band[entry->band]);
#endif /* BND_STRG_QA */
...
void bndstrg_periodic_exec(void *eloop_data, void *user_ctx)
{
	...
	for (i = 0; i < BND_STRG_MAX_TABLE_SIZE; i++)
	{
		...
		if (entry->bValid == TRUE)
		{
			...
			if(entry->band == 0 && elapsed_time >= table->DormantTime)
			{
				if (table->dbdc_mode == 1) {
			...
			}else if (entry->band != 0 && elapsed_time >= table->AgeTime)
			{
				if (table->dbdc_mode == 1) {
					bndstrg_accessible_cli(bndstrg, table->uc2GIfName, entry, CLI_AGING_REQ);
					bndstrg_accessible_cli(bndstrg, table->uc5GIfName, entry, CLI_AGING_REQ);
