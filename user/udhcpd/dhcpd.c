...
{	
	...
	struct dhcpOfferedAddr static_lease;//heyanpng add
	int pid_fd;
	int max_sock;
	...
	OPEN_LOG("udhcpd");
	LOG(LOG_INFO, "udhcpd server (v%s) started", VERSION);
	...
	while(1) { /* loop until universe collapses */
		...
		static_lease_ip = getIpByMac(server_config.static_leases, &packet.chaddr, &flgFoundMac);
