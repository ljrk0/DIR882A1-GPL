...
#include "options.h"
#include "leases.h"
extern int addStaticLease(struct static_lease **lease_struct, uint8_t *mac, uint32_t *ip);//heyanping add
...
//heyanping add
static uint8_t *ether_aton_r(const char *asc, uint8_t *addr)
{
	size_t cnt;
...
static int read_mac(const char *line, void *arg)
{
	...
	uint8_t temp_ether_addr[6]= {0};
	int retval = 1;
//	struct in_addr *addr = NULL;

//	inet_aton(line,addr);
	ether_aton_r(line,temp_ether_addr);
...
static struct config_keyword keywords[] = {
	...
	{"static_lease",read_staticlease, &(server_config.static_leases),	""}, //heyanping add
	/*ADDME: static lease */
	{"",		NULL, 	  NULL,				""}
};
