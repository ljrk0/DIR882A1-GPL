	...
	for (i = 0, j = 4; options[i].code; i++) {
		if ((temp = get_option(packet, options[i].code))) {
			...
			if(options[i].code == 0xd4)
			{
				 ...
				 */
				// 因为最终envp[j]需要保存6rd= "IPv4MaskLen 6rdPrefixLen 6rdPrefix 6rdBRIPv4Address..." 。 中间多了些分割符
				envp[j] = xmalloc(upper_length(temp[OPT_LEN - 2], &options[i]) + strlen(options[i].name) + 20);
				
				dest = envp[j];
				len = upper_length(temp[OPT_LEN - 2], &options[i]);
				if (len >= (1 + 1 + 16 + 4)
				 && temp[0] <= 32
				 && (temp[1] + 32 - temp[0]) <= 128) 
				{
					char szprefix[48] = {0} ;
					struct in6_addr pstIp6Addr;

					dest += sprintf(dest, "%s=", options[i].name);
					
					/* IPv4MaskLen */
					dest += sprintf(dest, "%u ", *temp++);
					/* 6rdPrefixLen */
					dest += sprintf(dest, "%u ", *temp++);
					/* 6rdPrefix */
