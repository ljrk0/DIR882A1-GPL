...
uint16_t				l2tp_opt_pre_tunnle_id = 0;
...
int main(int argc, char **argv)
{
	...
	openlog("[L2TPD]", log_flags, l2tp_opt_log_facility);
	if (l2tp_opt_debug) {
...
static int l2tp_parse_args(int argc, char **argv)
{
	...
	while((opt = getopt(argc, argv, "p:d:u:L:c:fRDSy:P:hI:")) != -1) {
		switch(opt) {
		case 'h':
			usage(argv, 0);
			break;
		...
		case 'I':
			...
			l2tp_opt_pre_tunnle_id = atoi(optarg) > 0 ? atoi(optarg) : 0;
			break;
