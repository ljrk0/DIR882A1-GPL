...
static struct mrib_iface* mrib_get_iface(int ifindex)
{
	...
	get_addr_by_interface_index(ifindex, &addr);
...
int get_addr_by_interface_index(int index, unsigned long *addr)
{
	int s = -1;
	struct ifreq ifr;
...
void mrib_delete_routes(int ifindex, const struct in6_addr *group)
{
	...
	if (mifid < MAXMIFS) {
		struct mrib_route *c, *n;
		list_for_each_entry_safe(c, n, &mifs[mifid].routes, head) {
			if (0 == memcmp(&c->group, group, sizeof (struct in6_addr))) {
				mrib_set(&c->group, &c->source, &mifs[mifid], 0, 1);
				list_del(&c->head);
				free(c);
			}
		}
