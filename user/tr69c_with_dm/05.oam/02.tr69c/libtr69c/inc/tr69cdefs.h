...
typedef enum {
   ...
   rpcAutonomousTransferCompleteResponse,
   rpcAutonomousDUStateChangeCompleteResponse,
   rpcVendorSpecific
} eRPCMethods;
...
typedef struct ACSState {
    ...
    char        *acsUser;
    char        *acsPwd;
    unsigned char  diffServ;    /* dscp field in ip header */
    ...
    char        *filename;      /* the file uploaded/downloaded, only for autonomous transfer */
    eFileType   filetype;       /* file type, only for autonomous transfer */
    unsigned int filesize;      /* file size, only for autonomous transfer */
    char        *announceURL;   /* the url which has file transfer request, only for autonomous transfer  */
    char        *transferURL;   /* the url which is used to upload/download file, only for autonomous transfer  */
    char        autonomousDl;   /* autonomous file upload/download */
    char        isDownload;   /* file upload/download */
    ...
    char         SelfDefinedPrefix[32]; /**< self defined prefix */
    ...
    int         enableSelfdefinedEventCode;
    unsigned int         tlsVersion;/* tls version */
    char *               tlsAuthenticationMethod;/* cert instance */
    char *               tlsPeerAuthenticationMethod;/* ca instance */
    char *               tlsCiphers;
    char *               tlsCertDir;
    char *               tlsClientCertFile;
    char *               tlsClientPrivateKeyFile;
    char *               tlsCaCertFile;
    char                 tlsCaCertRequired;
} ACSState;
...
#define INFORM_EVENT_VENDOR_ALARM           152
...
typedef struct transfer_complete_ind
{
    UINT32      status;         /* 0:not-sent, 1:sending, 2:sent */
    time_t      startDLTime;    /* start download time */
    time_t      endDLTime;      /* complete download time*/
    int         dlFaultCode;  /* download fault status */
    char        *dlFaultMsg;    /* download fault message */
    UBOOL8      bDownload;      /* download or upload */
    UBOOL8      bAutonomous;    /* autonomous or request */
    char        filename[256];      /* local filename */
    UINT32      filesize;       /* file size */
    eFileType   filetype;       /* file type, only for autonomous transfer */
    char        announceURL[1024];   /* the url which has file transfer request, only for autonomous transfer  */
    char        transferURL[1024];   /* the url which is used to upload/download file, only for autonomous transfer  */
    struct transfer_complete_ind *Next;
}TransferCompleteInd;
