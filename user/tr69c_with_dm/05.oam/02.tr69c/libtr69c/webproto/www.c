...
int www_EstablishConnection(struct sockaddr_storage host_addr, int *sock_fd)
{
  ...
  {
     cmsLog_error(TR069_ALARM_PARAMETER_ERROR,"neither af_inet nor af_inet6!");
     return -1;
  }
  ...
  {
      res = setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, acsState.boundIfName, strlen(acsState.boundIfName)+1);
      if (res < 0)
      {
  ...
  diffServ = acsState.diffServ;
  diffServ = (diffServ<<2)&0xfc;
  if(setsockopt(fd, IPPROTO_IP, IP_TOS, (void *)&diffServ, sizeof(diffServ)) < 0)
  {
    cmsLog_error(TR069_ALARM_PARAMETER_ERROR,"setsockopt() IP_TOS failed diffServ=0x%x",diffServ);
    close(fd);
    return -2;
  }
  else
  {
    cmsLog_notice("inform socket created with diffserv(0x%x)",diffServ);
  }
  ...
  if (connect(fd, (struct sockaddr *)&host_addr, saLen) < 0) {
    if (errno != EINPROGRESS) {
      /* connect failed */
      close(fd);
      cmsLog_notice("inform socket connecting failed, error %d %s",errno,strerror(errno));
      return -2;
    }
  }
#if 0
