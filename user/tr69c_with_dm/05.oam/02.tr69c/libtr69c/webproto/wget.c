...
static void freeHdrs( XtraPostHdr **p )
{
	XtraPostHdr *next = *p;
        while( next ){
            XtraPostHdr *temp;
            temp = next->next;
            CMSMEM_FREE_BUF_AND_NULL_PTR(next->hdr);
...
static void freeData(tWgetInternal *wg)
{
    if (wg != NULL) {
        ...
        CMSMEM_FREE_BUF_AND_NULL_PTR(wg->host);
        CMSMEM_FREE_BUF_AND_NULL_PTR(wg->uri);
        if (wg->hdrs != NULL) 
        ...
        CMSMEM_FREE_BUF_AND_NULL_PTR(wg);
    }
}
		...
		if (replaceDups == REPLACE) {
			while (*p) {
				...
				if ( strcasecmp(xp->name, xh->name)==0) {
					/* replace header */
					xh->next = xp->next;
					CMSMEM_FREE_BUF_AND_NULL_PTR(xp->name);
					CMSMEM_FREE_BUF_AND_NULL_PTR(xp->value);
					CMSMEM_FREE_BUF_AND_NULL_PTR(xp);
					*p = xh;
					return 1;
				}
				p = &xp->next;
		...
		if (replaceDups == REPLACE) {
			while (*p) {
				...
				if ( strcmp(xp->hdr, xh->hdr)==0) {
					/* replace header */
					xh->next = xp->next;
					CMSMEM_FREE_BUF_AND_NULL_PTR(xp->hdr);
					CMSMEM_FREE_BUF_AND_NULL_PTR(xp->value);
					CMSMEM_FREE_BUF_AND_NULL_PTR(xp);
					*p = xh;
					return 1;
...
static void do_send_request(void *p, int errorcode)
{
   ...
   ret = cmsTmr_set(tmrHandle, timer_response, data, ACSRESPONSETIME, "timer_response"); /* response timeout is 30 sec */
   if (ret != CMSRET_SUCCESS)
   {
      cmsLog_error(TR069_ALARM_TIMER_ERROR,"could not set ACS response timer, ret=%d", ret);
   }
...
static void do_response(void *p)
{
   ...
   if (data->pc == NULL)
   {
      cmsLog_error(TR069_ALARM_PROCESS_ERROR,"wget %s", "Internal Error");
      report_status(data, iWgetStatus_InternalError, 
   ...
   {
      /* memory exhausted?!? */
      cmsLog_error(TR069_ALARM_PROCESS_ERROR,"wget %s", "Memory exhausted");
      return;
   }
...
tWgetInternal *wget_Connect(const char *url, CmsEventHandler callback, void *handle)
{
   ...
   if (wg->port == 0)
   {
      ...
      case URL_PROTO_HTTPS:
         ...
         break;
      default:
         cmsLog_error(TR069_ALARM_PARAMETER_ERROR,"unsupported protocol urlProto=%d (default to port 80)", wg->urlProto);
   ...
   if (dns_lookup(wg->host, &(wg->host_addr)))
   {
      ...
      { /* connection complete- start it */
         ...
         res = cmsTmr_set(tmrHandle, (CmsEventHandler)timer_connect, (void *)wg, ACSRESPONSETIME, "timer_connect"); /* response timeout is 30 sec */
         if (res != CMSRET_SUCCESS)
         {
            cmsLog_error(TR069_ALARM_PROCESS_ERROR,"could not set ACS connect() timer, res=%d", res);
         }
      }
   }
...
void wget_ClearPostHdrs( tWgetInternal *wio)
{
   ...
   {
      XtraPostHdr *nxt;
      CMSMEM_FREE_BUF_AND_NULL_PTR( xh->hdr);
      CMSMEM_FREE_BUF_AND_NULL_PTR(xh->value);
      nxt = xh->next;
      CMSMEM_FREE_BUF_AND_NULL_PTR(xh);
      xh= nxt;
   }
