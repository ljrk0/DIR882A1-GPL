...
void freeAllListeners(void)
{
   ...
   {
      tmp = l->next;
      CMSMEM_FREE_BUF_AND_NULL_PTR(l);
      l = tmp;
   }
}
...
void setListenerType(int fd, CmsEventHandler func, void* handle, tListenerType type)
{
   ...
   {
      cmsLog_error(TR069_ALARM_SOCKET_ERROR,"Suspicious fd=%d handle=%p type=%s", fd, handle, listenerType);
   ...
   if(l==NULL)
   {
      cmsLog_error(TR069_ALARM_MEMORY_ALLOC_FAILED,"malloc() failed for l");
      return;
   }
   l->fd     = fd;
   l->func   = func;
...
void eventLoop(void)
{
   ...
   while (keepLooping)
   {
      ...
      for (l = listeners; l; l = l->next)
      {
         ...
         {
            cmsLog_error(TR069_ALARM_SOCKET_ERROR,"Don't set fd=%d", l->fd);
            continue;
         }
         ...
         switch (l->type)
         {
            ...
            case iListener_Except:
               ...
               break;
            default:
               cmsLog_error(TR069_ALARM_SOCKET_ERROR,"Impossible error: illegal listener type (%d)", l->type);
               break;
         }
         if (n <= l->fd)
         {
      ...
      else
      {
         ...
         cmsLog_notice("set select timeout to %d.%03d secs",
      ...
      {
         cmsLog_error(TR069_ALARM_SOCKET_ERROR,"Error in select (%d=%s)", errno, strerror(errno));
         continue;
      }
      ...
      {
         cmsLog_notice("select() timeout, the interval is %d.%03d secs", tm.tv_sec, (tm.tv_usec / USECS_IN_MSEC));
      }
