...
//   struct in_addr inAddr;
   ...
   while (!found && 
          cmsObj_getNext(MDMOID_IPV6_HOST_CONFIG_DATA,
                          &iidStack, (void **) &ipv6Host) == CMSRET_SUCCESS)
   {
      ...
      if (cmsUtl_strcmp(ipv6Address, ipAddr) == 0)
      {
         ...
         pathDesc.oid = MDMOID_IPV6_HOST_CONFIG_DATA;
...
#endif    // SUPPORT_IPV6
   while (!found && 
          cmsObj_getNext(MDMOID_IP_HOST_CONFIG_DATA,
                          &iidStack, (void **) &ipHost) == CMSRET_SUCCESS)
   {
      ...
      if (cmsUtl_strcmp(inet_ntoa(inAddr), ipAddr) == 0)
      {
         ...
         pathDesc.oid = MDMOID_IP_HOST_CONFIG_DATA;
         pathDesc.iidStack = iidStack;
