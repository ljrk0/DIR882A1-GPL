...
void set_wan(char *input)
{
	...
	if (!strncmp(l2tp_opmode, "KeepAlive", 9))
		WebTF(input, "l2tpRedialPeriod", RT2860_NVRAM, "wan_l2tp_optime", 1);
#if 0
...
void set_lan(char *input)
{
	...
	if(lan2enabled == NULL) lan2enabled="";
	lan2_ip = strdup(web_get("lan2Ip", input, 1));
	if(lan2_ip == NULL) lan2_ip="";
...
#if defined (CONFIG_RALINK_MT7621) || defined (CONFIG_ARCH_MT7623) || defined (CONFIG_GE_RGMII_MT7530_P0_AN) || defined(CONFIG_GE_RGMII_MT7530_P4_AN) || defined (CONFIG_P5_RGMII_TO_MT7530_MODE)	
	WebTF(input, "eeeEnbl", RT2860_NVRAM, "eeeEnabled" ,1);
#endif
	dhcp_s = strdup(web_get("dhcpStart", input, 1));
	if(dhcp_s == NULL) dhcp_s="";
	dhcp_e = strdup(web_get("dhcpEnd", input, 1));
	if(dhcp_e == NULL) dhcp_e="";
	dhcp_m = strdup(web_get("dhcpMask", input, 1));
	if(dhcp_m == NULL) dhcp_m="";
...
void set_add_routing(char *input)
{
	...
	if(custom_interface == NULL) custom_interface="";
	...
	// netmask
	if(strlen(netmask))
		snprintf(cmd, sizeof(cmd), "%s netmask %s", cmd, netmask);
	else
		strcpy(netmask, "255.255.255.255");

	//gateway
	if(strlen(gateway))
		snprintf(cmd, sizeof(cmd),"%s gw %s", cmd, gateway);
	else
	...
	snprintf(cmd, sizeof(cmd), "%s dev %s ", cmd, true_interface);
	...
	if(!strlen(result)){
		...
		if(!rrs || !strlen(rrs)){
			...
			strlcpy(tmp, rrs, sizeof(tmp));
		}
		if(strlen(tmp))
			strcat(tmp,";");
		snprintf(tmp, sizeof(tmp),"%s%s,%s,%s,%s,%s,%s,%s", tmp, dest, netmask, gateway, interface, true_interface, custom_interface, comment);
...
{
	char cmd[CMDLEN];
	strlcpy(cmd, "route del ", sizeof(cmd));
	...
	//interface
	snprintf(cmd, sizeof(cmd), "%s dev %s ", cmd, ifname);
	system(cmd);
}
...
void set_delete_routing(char *input)
{
	...
	for(index=0; index< rule_count; index++){
		...
		if(value){
			...
			if(strlen(value) >= 256)
				continue;
			sscanf(value, "%256s%256s%256s", dest, netmask, true_interface);
			DBG_MSG("dest=%s\n", &dest[0]);
			DBG_MSG("netmask=%s\n", &netmask[0]);
			DBG_MSG("true_interface=%s\n", &true_interface[0]);
			remove_routing_rule(&dest[0], netmask, true_interface);
...
void config_ipv6(int mode)
{
	...
	if (prefix_len >= 256) prefix_len = 256; 
	...
	case 2:
		...
		snprintf(v6addr, sizeof(v6addr),"%x", temp[0]);
		for (used=1; used<i; used++)
			snprintf(v6addr, sizeof(v6addr),"%s:%x", v6addr, temp[used]);
		...
		snprintf(ipv6_ip_addr, sizeof(ipv6_ip_addr),"%x", temp[0]);
		for (i=1; i<used; i++) {
			if (i>=7) i=7;
			snprintf(ipv6_ip_addr, sizeof(ipv6_ip_addr),"%s:%x", ipv6_ip_addr, temp[i]);
		}
...
void static_routing_init(void)
{
	...
	while (fgets(one_rule, sizeof(one_rule), fp) != NULL) {
		if (nl) {
			if (sscanf(one_rule, "%32s%lx%lx%X%d%d%d%lx",
			...
			strlcpy(dest, inet_ntoa(dest_s), sizeof(dest));
		
			strlcpy(gw, inet_ntoa(gw_s), sizeof(gw));
		
			strlcpy(netmask, inet_ntoa(netmask_s), sizeof(netmask));
	...
	while( get_nth_value(index, rrs, ';', one_rule, 256) != -1 ){
		...
		if(strlen(gw) && strcmp(gw, "0.0.0.0"))
			snprintf(cmd, sizeof(cmd),"%s gw %s", cmd, gw);
...
leave:
	if(NULL != inStr)
		free(inStr);
	tmp = nvram_get(RT2860_NVRAM, "lan_ipaddr");
	if (tmp == NULL)
		return 0;
	printf("webs: Listening for HTTP requests at address %s", tmp);	
