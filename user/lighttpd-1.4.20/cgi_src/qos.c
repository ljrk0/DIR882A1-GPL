...
static void qos_port_setup(char *input)
{
	...
	for(i=0; i<13; i++){
		if(portX[i]){
			...
			snprintf(tmp_str, sizeof(tmp_str), "%s%s", tmp_str,  portX[i]);
		}
		//strcat(tmp_str, ",");
		//strncat(tmp_str, ",", sizeof(tmp_str));
		
		if(portX_remark[i]){
			//strcat(tmp_str, portX_remark[i]);
			//strncat(tmp_str, portX_remark[i], sizeof(tmp_str));
			snprintf(tmp_str, sizeof(tmp_str), "%s%s", tmp_str, portX_remark[i]);
		}
		//strcat(tmp_str, ",");
		//strncat(tmp_str, ",", sizeof(tmp_str));
		snprintf(tmp_str, sizeof(tmp_str), "%s%s", tmp_str, ",");
	}
...
static void qos_setup(char *input)
{
	...
	if( NULL == upload_bandwidth) upload_bandwidth=""; 
    	if( NULL == upload_bandwidth_custom ) upload_bandwidth_custom="";
    	if( NULL == download_bandwidth) download_bandwidth=""; 
  	if( NULL == download_bandwidth_custom ) download_bandwidth_custom="";
	...
	if(!strcmp(qos_enable, "1") /* bi-dir */ || !strcmp(qos_enable, "2")/* upload */ || !strcmp(qos_enable, "3") /* download */ || !strcmp(qos_enable, "4") /* port based */){
		...
		if(!strchr(&postfix[0], 'k') && !strchr(&postfix[0], 'K')  && !strchr(&postfix[0],'m') && !strchr(&postfix[0],'M') ){
			snprintf(postfix, sizeof(postfix), "%s%s", postfix, "k");
			//strncat(postfix, "k", sizeof(postfix));
		}
		
		nvram_bufset(RT2860_NVRAM, "QoSUploadBandwidth_custom", &postfix[0]);
		strncpy(postfix, download_bandwidth_custom, sizeof(postfix));
		if(!strchr(&postfix[0], 'k') && !strchr(&postfix[0],'K')  && !strchr(&postfix[0],'m') && !strchr(&postfix[0],'M') )
			snprintf(postfix, sizeof(postfix), "%s%s", postfix, "k");
			//strncat(postfix, sizeof(postfix), "k");

		nvram_bufset(RT2860_NVRAM, "QoSDownloadBandwidth_custom", &postfix[0]);
...
*/	
leave:
	free_all(24, qos_enable, qos_model, upload_bandwidth, download_bandwidth, 
			upload_bandwidth_custom, download_bandwidth_custom, 
			...
			low_download_queue_rate, low_download_queue_ceil, reserve_bandwidth, qos_type);
	return;
}
/*
...
/*
	inStr = malloc(inLen);
	if (inStr == NULL){
		DBG_MSG("malloc failed. quit program");
		web_debug_header();
		printf("malloc failed. quit cgi");
		goto leave;
	}*/
