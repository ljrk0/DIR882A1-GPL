...
void set_opmode(char *input)
{
	...
	const char *old_boost = nvram_bufget(RT2860_NVRAM, "boost_mode");
	int need_commit = 0;
//#if defined CONFIG_RT2860V2_STA_DPB || defined CONFIG_RT2860V2_STA_ETH_CONVERT
#if defined (RT2860_STA_SUPPORT) && defined (ETH_CONVERT_SUPPORT)
	...
	tmp = nvram_get(RT2860_NVRAM, "TcpTimeout");
	if (tmp == NULL) tmp ="";
	if (strcmp(value, tmp)) {
	...
	tmp = nvram_get(RT2860_NVRAM, "UdpTimeout");
	if (tmp == NULL) tmp ="";
	if (strcmp(value, tmp)) {
...
//#if defined CONFIG_RT2860V2_STA_DPB || defined CONFIG_RT2860V2_STA_ETH_CONVERT
	...
	if (!strncmp(mode, "0", 2)) {
		...
		WebNthTF(input, "apcliEnbl", RT2860_NVRAM, 0, "ApCliEnable", 1);
		need_commit = 1;
	}
#endif
	...
	if (!strncmp(mode, "0", 2)) {
		nvram_bufset(RTDEV_NVRAM, "apClient", web_get("apcliEnbl", input, 1));
		WebNthTF(input, "apcliEnbl", RTDEV_NVRAM, 0, "ApCliEnable", 1);
		need_commit = 1;
	}
#endif
	...
	value = web_get("boost_mode", input, 1);
	if (strcmp(value, old_boost)) {
		nvram_bufset(RT2860_NVRAM, "boost_mode", value);
		need_commit = 1;
	}
...
//#if defined CONFIG_RT2860V2_STA_DPB || defined CONFIG_RT2860V2_STA_ETH_CONVERT
	...
	if (need_commit) {
		...
		//update_flash_8021x(WIFI3_NVRAM);
