...
#else
#define DEFAULT_CONFPATH "/etc/stupid-ftpd.conf"
#endif
...
void set_path(char *arg)
{
    ...
    if(logininfocount < LOGININFO_COUNT) {
        if(logininfo[logininfocount] == NULL) {
            printf("TW Warning: out of memory.\n");
            return;
        }

        buf_len = sizeof(logininfo[logininfocount]->root) - 1;
        strncpy(logininfo[logininfocount]->root, arg, buf_len);
        logininfo[logininfocount]->root[buf_len] = '\0';

        logininfocount++;
    } else
        printf("TW Warning: Too many users (>%d) specified.\n", LOGININFO_COUNT);

    return;    

}

int split_ftp_user_rules(char *rule, char **szRule, char szDelimit, int max_rule_num)
{
	char *p_result = NULL;
	int i = 0;
	char *pos1 = NULL, *pos2 = NULL;
	
	if(NULL == rule || NULL == szRule || max_rule_num <= 0) {
		return -1;
	}
	...
	while(pos2 = strchr(pos1, szDelimit)) {
        *pos2 = '\0';
        *(szRule + i) = pos1;
        pos1 += strlen(pos1) + 1;
        if(++i == max_rule_num - 1) {
...
void add_user(char *arg) {
  char perm[40];
    ...
    char split = ';';
    char *ftp_user_info_list[8] = {NULL};
#endif
...
#ifdef TW_MODIFY
    if (arg == NULL || split_ftp_user_rules(arg, ftp_user_info_list, split, FTP_ARG_NUM) == -1) {
        printf("TW Invalid user registration format.\n");
        printf("TW The correct format is: username;maxlogins;perm;password\n");
        free(logininfo[logininfocount]);
        logininfo[logininfocount] = NULL;
        return;
    }

    snprintf(logininfo[logininfocount]->username,
             sizeof(logininfo[logininfocount]->username),
             "%s",
             ftp_user_info_list[FTP_USERNAME]);

    logininfo[logininfocount]->maxlogins = atoi(ftp_user_info_list[FTP_MAX_LOGINS]);

    snprintf(perm,
             sizeof(perm),
             "%s",
             ftp_user_info_list[FTP_PERM]);   

    snprintf(logininfo[logininfocount]->passwd,
             sizeof(logininfo[logininfocount]->passwd),
             "%s",
             ftp_user_info_list[FTP_PASSWORD]);  
