...
if [ "$PRODUCT_NAME" = "DIR-867" -o "$PRODUCT_NAME" = "DIR-1935" ]; then
	...
	if [ "$PRODUCT_NAME" = "DIR-1935" ]; then
		PRODUCT_ID="AP-MSM001-0014-dlink-sdk-"
		else
		PRODUCT_ID="AP-MSM001-0001-dlink-sdk-"
		fi	
else
...
BUILD_DAY=_`date "+%Y-%m-%d"`
BUILD_HOUR=_`date |cut -d ' ' -f  5`
...
echo '#define TW_SW_VERSION_EXT' \"$PRODUCT_ID$PRJ_SVN_REVISION$BUILD_DAY$BUILD_HOUR\" >> $SOFTWARE_VER_FILE
