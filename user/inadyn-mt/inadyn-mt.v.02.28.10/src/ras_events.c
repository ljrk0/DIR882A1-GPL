...
static RTL_OSVERSIONINFOEXW	*p_os_ver=NULL;
...
static unsigned __stdcall wait_for_ras_notifications_thread(void *p_thread_data)
{
...
static unsigned __stdcall manage_ras_thread(void *thread_data)
{
	...
	p_ras_thread_data->ras_notifications_thread=begin_thread(wait_for_ras_notifications_thread,thread_data);

	if (!(p_ras_thread_data->ras_notifications_thread)) {
...
{
	p_os_ver=safe_malloc(sizeof(RTL_OSVERSIONINFOEXW));

	get_os_version(p_os_ver);

		if (DEBUG_LEVEL>=LOG_DEBUG)

		DBG_PRINTF((LOG_DEBUG,"I:" MODULE_TAG "OS major version, %d, minor version, %d\n",p_os_ver->dwMajorVersion,p_os_ver->dwMinorVersion));
...
void ras_set_debug_level(RAS_THREAD_DATA *p_ras_thread_data,DWORD debug_level)
{
...
unsigned __stdcall launch_ras_events_trap(void **p_data)
{
	...
	if (!(begin_thread(manage_ras_thread,p_ras_thread_data)==-1)) {
...
BOOL trap_ras_events(RAS_THREAD_DATA **p_ras_thread_data,BOOL init_wait)
{
	...
	if (begin_thread(launch_ras_events_trap,p_ras_thread_data)==-1)
...
RAS_THREAD_DATA* construct_trap_ras_events(EVENT_TRAP_HANDLER p_func,void *p_context,const char *test_url,DWORD debug_level)
{
	...
	if (!(init_thread_data(p_ras_thread_data))) {
		...
		ras_set_debug_level(p_ras_thread_data,debug_level);
