...
static void do_stream_wave(WAVE_PLAY *p_wave_play,int bytes_read)
{
	int		cur_buffer;
...
static void stream_wave(WAVE_PLAY *p_wave_play)
{
	...
	/*Seeding before stream wave loop seems to dramatically improve
	  performance, and the need for much buffering - related semaphore
	  must not be created signaled.  Windows likes both buffers before
	  waiting on the semaphore for the first time.*/
	is_done=fill_wave_buffer(p_wave_play,p_wave_play->cur_buffer,&bytes_read);
	do_stream_wave(p_wave_play,bytes_read);
...
static WAVE_PLAY *create(WAVE_PLAY **p_wave_play,WAVE_PARAMS *p_wave_params)
{
	...
	p_this_wave->buffer_size=(int) (p_this_wave->sample_rate*(!(p_wave_params->buff_factor) ?
		.25:p_wave_params->buff_factor)*p_this_wave->output_factor);
