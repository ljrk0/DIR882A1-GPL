...
/*only short options as user input.  long options were added
  so that service client programs may uniformly convert user
  input short options (single hyphen) to long options.*/
static CMD_DESCRIPTION_TYPE service_cmd_options_table[] =
{
        {SVC_LONG_OPTION_INSTALL,1,{service_install_handler,NULL,1},"install service"},
	{"-i",1,{service_install_handler,NULL,1},"install service"},
	{SVC_LONG_OPTION_NAME,1,{service_name_handler,NULL,1},"service name"},
	{"-n",1,{service_name_handler,NULL,1},"service name"},
        {SVC_LONG_OPTION_UNINSTALL,0,{service_uninstall_handler,NULL,0},"uninstall service" },
        {"-x",0,{service_uninstall_handler,NULL,0},"uninstall service" },
        {SVC_LONG_OPTION_START,0,{service_start_handler, NULL,0},"start service" },
	{"-s",0,{service_start_handler, NULL,0},"start service" },
        {SVC_LONG_OPTION_EXIT,0,{service_exit_handler, NULL,0},"exit service" },
	{"-e",0,{service_exit_handler, NULL,0},"exit service" },
        {SVC_LONG_OPTION_RESTART,0,{service_restart_handler, NULL,0},"restart service" },
...
static unsigned __stdcall service_work_thread(void *worker_args)
{
	...
	service_client(p_worker_args->argc,utf_16_to_8_ar(utf_8_ar,dest,p_worker_args->argc),1,pLOG_FILE);
...
static void WINAPI ServiceMain(DWORD dwArgc,LPTSTR lpszArgv[])
{
	...
	*/
	...
	if (!(hService)) {
		...
		hServiceWorkThread=begin_thread(service_work_thread,&main_args);

		if (!(hServiceWorkThread)) {
			...
			if (!(WaitForSingleObject((HANDLE) hServiceWorkThread,INFINITE))==WAIT_OBJECT_0)
				...
				CloseHandle((HANDLE) hServiceWorkThread);
				hServiceWorkThread=0;
			}
...
static BOOL install_add_name_param()
{
	...
	swcQuotedName=safe_malloc((wcslen(szServiceName)+3)*sizeof(wchar_t));

	swprintf(swcQuotedName,L"\"%s\"",szServiceName);

	swcImagePathParamed=safe_malloc((wcslen(swcImagePath)+wcslen(L" ")+wcslen(_SERVICE_NAME)+wcslen(L" ")+wcslen(swcQuotedName)+1)*sizeof(wchar_t));

	wcscat(wcscat(wcscat(wcscat(wcscpy(swcImagePathParamed,swcImagePath),L" "),_SERVICE_NAME),L" "),swcQuotedName);
					...
					FILE *p_log_file)
{
	...
	if (!(isService()) || !(isWinNT())) {
		...
		service_client_callback(argc,utf_8_argv,0,pLOG_FILE);
	}
	else {
