...
int stad_get_client_type(char *mac_addr)
{
	FILE *file = NULL;
	char buf[256] = { 0 };
	char szcmd[128] = {0};
	...
	snprintf(sztmp,sizeof(sztmp),"iwpriv %s get_mac_table",INTERFACE_2G);
	file = popen(sztmp, "r");
	if(file != NULL)
	{
		while(NULL != fgets(buf, sizeof(buf), file))
		{
			if(NULL != strstr(buf, mac_addr))
			{
				type = 1;
				pclose(file);
				goto exit;
	...
	snprintf(sztmp,sizeof(sztmp),"iwpriv %s get_mac_table",INTERFACE_5G);
	file = popen(sztmp, "r");
	if(file != NULL)
	{
		while(NULL != fgets(buf, sizeof(buf), file))
	...
	snprintf(sztmp,sizeof(sztmp),"iwpriv %s get_mac_table",INTERFACE_2G_GUEST1);
	file = popen(sztmp, "r");
	if(file != NULL)
	{
		while(NULL != fgets(buf, sizeof(buf), file))
	...
	snprintf(sztmp,sizeof(sztmp),"iwpriv %s get_mac_table",INTERFACE_5G_GUEST1);
	file = popen(sztmp, "r");
	if(file != NULL)
	{
		while(NULL != fgets(buf, sizeof(buf), file))
...
int stad_get_type_by_mac(char *mac, ST_CLIENT_INFO *pst_client_info)
{
		...
		type = stad_get_client_type(mac);
		
		switch(type)
		{
			case TW_USER_DEVICE_TYPE_5G:   // 2
					strcpy(pst_client_info->Type, "WiFi_5G");
					break;
			case TW_USER_DEVICE_TYPE_24G: // 1
	...
	if(ifr.ifr_flags & IFF_UP) {
		sprintf(szCmd,"iwpriv %s get_mac_table",INTERFACE_2G);
		file = popen(szCmd, "r");
		if(file != NULL)
		{
			while(NULL != fgets(buf, sizeof(buf), file))
	...
	ioctl(s, SIOCGIFFLAGS, &ifr);
	if(ifr.ifr_flags & IFF_UP) {
		sprintf(szCmd,"iwpriv %s get_mac_table",INTERFACE_5G);
		file = popen(szCmd, "r");
		if(file != NULL)
		{
			while(NULL != fgets(buf, sizeof(buf), file))
	...
	ioctl(s, SIOCGIFFLAGS, &ifr);
	if(ifr.ifr_flags & IFF_UP) {
		sprintf(szCmd,"iwpriv %s get_mac_table",INTERFACE_2G_GUEST1);
		file = popen(szCmd, "r");
		if(file != NULL)
		{
			while(NULL != fgets(buf, sizeof(buf), file))
	...
	ioctl(s, SIOCGIFFLAGS, &ifr);
	if(ifr.ifr_flags & IFF_UP) {
		sprintf(szCmd,"iwpriv %s get_mac_table",INTERFACE_5G_GUEST1);
		file = popen(szCmd, "r");
		if(file != NULL)
		{
			while(NULL != fgets(buf, sizeof(buf), file))
...
1.先查/proc/net/arp表项
2.再通过arp去查询是否在线
3.如果arp查询不到，则采用查询无线mac表项去查，防止部分设备连上但是不响应arp
*/
...
void add_wireless_stainfo(void)
{
	FILE *file = NULL;
	char buf[256] = { 0 };
	char szcmd[128] = {0};
	...
	//char MacBuf[6] = {0};
	int i=0;
	char szInterface[][6]={INTERFACE_2G,INTERFACE_5G,INTERFACE_2G_GUEST1,INTERFACE_5G_GUEST1};

	const char *pattern = "^[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}$";
	
	for(i=0;i<sizeof(szInterface)/sizeof(szInterface[0]);i++)
	{	
		snprintf(sztmp,sizeof(sztmp),"iwpriv %s get_mac_table",szInterface[i]);
		file = popen(sztmp, "r");
		if(file != NULL)
		{
			while(NULL != fgets(buf, sizeof(buf), file))
			{
				strncpy(szMac,buf,17);
				if(TRUE == tbsMatch(szMac, pattern))
					stad_add_mac(szMac);
