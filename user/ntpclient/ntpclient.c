...
#undef Data
	...
	if (set_clock) {   /* you'd better be root, or ntpclient will crash! */
        ...
        // check daylight token
        int daylight = atoi(nvram_safe_get("ntp_DaylightSavingsUsed"));
        // shift time by token*3600
    	if (strchr(pszTZOffset, '-'))
    	{
    		sscanf(pszTZOffset, "-%d:%d", &Hours, &Minutes);
            tv_set.tv_sec -= (Hours*60 + Minutes)*60;
    	}
    	else
    	{
    		sscanf(pszTZOffset, "+%d:%d", &Hours, &Minutes);
        ...
        // if daylight saveing enable, shift time by 3600
        if(daylight)
            tv_set.tv_sec += 3600;
