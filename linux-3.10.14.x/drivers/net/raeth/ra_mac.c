...
static int PortStatusRead(struct seq_file *seq, void *v)
{
	int len = 0;
	int port_no;
    ...
    defined (CONFIG_RALINK_MT7620)	
		...
		#endif
		if (reg_val & 0x1) {
			seq_printf(seq, "Port%d: LinkUp\n", port_no);
			//len += sprintf(page + len, "Port%d: LinkUp\n", port_no);
		} else {
			seq_printf(seq,"Port%d: LinkDown\n", port_no);
...
static ssize_t IntPortStatusOpen(struct inode *inode, struct file *file)
{
	//memset(&raeth_int, 0, sizeof(raeth_int));
	return single_open(file, PortStatusRead, NULL);
}

static const struct file_operations int_port_status_fops = {
	.owner 		= THIS_MODULE,
	.open       	= IntPortStatusOpen,
	.read	 	= seq_read,
	.write 		= PortStatusWrite
};
//PortStatusRead
...
#else
if (!(procPortStatus = proc_create(PROCREG_PORT_STATUS, 0, procRegDir, &int_port_status_fops)))
	 printk("!! FAIL to create %s PROC !!\n", PROCREG_PORT_STATUS);
#endif
