    ...
    IN PRTMP_ADAPTER pAd)
{
    ...
    for (i=0; i < MaxWcidNum; i++)
	{
		...
		if (IS_ENTRY_CLIENT(pEntry) && (pEntry->Sst == SST_ASSOC))/*??D¨¨¨¬?¦Ì?1?¨¢a¦Ì?¨¦¨¨¡À?¦Ì?sta¡ê?2?¨°a¡ã?apclii0¨°2¨¬?¦Ì?*/
		{
            ...
            Reason = REASON_DISASSOC_STA_LEAVING;
			
            MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ASSOC - Send DISASSOC  Reason = %d frame  TO %02x %02x %02x %02x %02x %02x \n",Reason,pEntry->Addr[0],
                pEntry->Addr[1],pEntry->Addr[2],pEntry->Addr[3],pEntry->Addr[4],pEntry->Addr[5]));

            MgtMacHeaderInit(pAd, &DisassocHdr, SUBTYPE_DISASSOC, 0, pEntry->Addr, 
                    pAd->ApCfg.MBSSID[pEntry->apidx].wdev.if_addr, pAd->ApCfg.MBSSID[pEntry->apidx].wdev.bssid);
            MakeOutgoingFrame(pOutBuffer, &FrameLen, sizeof(HEADER_802_11), &DisassocHdr, 2, &Reason, END_OF_ARGS);
            MiniportMMRequest(pAd, 0, pOutBuffer, FrameLen);
            MlmeFreeMemory( pOutBuffer);
