	...
	switch (MsgType)
	{
		...
		case WSC_MSG_M8:
			...
			if (pWscControl->WscState == WSC_STATE_WAIT_M8)
			{
				...
				/*MTK patch start by sudenghai 2014-12-8, 解决保持现有无线配置勾选，使用7610网卡Pin方式连接能强制修改无线配置*/
				if(pWscControl->WscConfStatus==WSC_SCSTATE_CONFIGURED)
  				{
				   MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WSC_SCSTATE_CONFIGURED\n"));
				   rv=WSC_ERROR_HMAC_FAIL;
				      goto Fail;
				}
				/*MTK patch end by sudenghai 2014-12-8*/
