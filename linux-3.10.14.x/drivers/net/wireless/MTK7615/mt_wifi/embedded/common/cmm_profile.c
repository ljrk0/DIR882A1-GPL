	...
	IN RTMP_STRING *pBuffer)
{
	...
	/*#endif*/
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			if (RTMPGetKeyParameter("AutoChannelSkipList", tmpbuf, 60, pBuffer, FALSE))//modify by taogan 2017/1/2
			{
				pAd->ApCfg.AutoChannelSkipListNum = delimitcnt(tmpbuf, ";") + 1;
				//if ( pAd->ApCfg.AutoChannelSkipListNum > 10 )
				if ( pAd->ApCfg.AutoChannelSkipListNum > 15 )//modify by taogan 2017/1/2
				{
					MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Your no. of AutoChannelSkipList( %d ) is larger than 15 (boundary)\n",pAd->ApCfg.AutoChannelSkipListNum));
					//pAd->ApCfg.AutoChannelSkipListNum = 10;
					pAd->ApCfg.AutoChannelSkipListNum = 15;//modify by taogan 2017/1/2
				}
