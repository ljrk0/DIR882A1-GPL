...
static void wifi_fwd_show_entry(void)
{
	...
	WF_FWD_PRINT(WF_DEBUG_OFF, ("driver version: %s (%s %s) .\n", FWD_VERSION, __DATE__, __TIME__));
	...
	for (idx=0; idx<WIFI_FWD_TBL_SIZE; idx++)
	{
		if (WiFiFwdBase[idx].valid)
			WF_FWD_PRINT(WF_DEBUG_OFF, ("[%s]index=%d, valid=%d, apcli-dev=0x%08x, ap-dev=0x%08x\n", 
	...
	for (idx=0; idx<WIFI_FWD_DEVICE_SIZE; idx++)
	{
		if (FwdDevice[idx].valid)
		{
			...
			if (FwdDevice[idx].band == band_2g)
				WF_FWD_PRINT(WF_DEBUG_OFF, ("band=2G,   src=0x%08x, ", (int)FwdDevice[idx].dev));
			else if (FwdDevice[idx].band == band_5g)
				WF_FWD_PRINT(WF_DEBUG_OFF, ("band=5G,   src=0x%08x, ", (int)FwdDevice[idx].dev));
			else if (FwdDevice[idx].band == band_5g_h)
				WF_FWD_PRINT(WF_DEBUG_OFF, ("band=5G-h, src=0x%08x, ", (int)FwdDevice[idx].dev));				

			if (FwdDevice[idx].partner != NULL)
				WF_FWD_PRINT(WF_DEBUG_OFF, ("partner=0x%08x\n", (int)FwdDevice[idx].partner->dev));
			else
	...
	for (k=0; k<WIFI_FWD_DEVICE_SIZE; k++)
	{
		...
		if (DeviceEntryOthDest != DeviceEntryDest)
		{
			...
			if (DeviceEntrySrc->band == band_5g)
			{
				...
				if (((DeviceEntryOthDest->band == band_2g) && (DeviceEntryOthDest->second_band == band_5g)) ||
			...
			else if (DeviceEntrySrc->band == band_5g_h)
			{
				...
				if (((DeviceEntryOthDest->band == band_2g) && (DeviceEntryOthDest->second_band == band_5g_h)) ||
