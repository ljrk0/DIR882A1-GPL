...
INT mt_ate_set_tmac_info(RTMP_ADAPTER *pAd, TMAC_INFO *tmac_info, UINT32 band_idx)
{
	...
	if (IS_MT7615(pAd) || IS_MT7622(pAd) ||	IS_P18(pAd) || IS_MT7663(pAd)) {
		if (ant_sel & ATE_ANT_USER_SEL) {
			...
			HW_IO_READ32(pAd, CR_WF3_RFINTF_00, &Value);
			Value &= ~CR_RFINTF_ENABLE_ANT_MASK;
			Value |= CR_RFINTF_ENABLE_ANT(0x0);
			HW_IO_WRITE32(pAd, CR_WF3_RFINTF_00, Value);

			HW_IO_READ32(pAd, CR_WF3_RFINTF_01, &Value);
			Value &= ~CR_RFINTF_ENABLE_ANT_MASK;
			Value |= CR_RFINTF_ENABLE_ANT(0x0);
			HW_IO_WRITE32(pAd, CR_WF3_RFINTF_01, Value);

			HW_IO_READ32(pAd, CR_WF3_RFINTF_02, &Value);
			Value &= ~CR_RFINTF_ENABLE_ANT_MASK;
			Value |= CR_RFINTF_ENABLE_ANT(0x0);
			HW_IO_WRITE32(pAd, CR_WF3_RFINTF_02, Value);

			HW_IO_READ32(pAd, CR_WF3_RFINTF_03, &Value);
			Value &= ~CR_RFINTF_ENABLE_ANT_MASK;
			Value |= CR_RFINTF_ENABLE_ANT(0x0);
			HW_IO_WRITE32(pAd, CR_WF3_RFINTF_03, Value);
		} else {
			INT map_idx = 0;
			...
			if (IS_MT7615(pAd))
				tmac_info->AntPri = 24;  /*fix to 0x18 for 7615*/
		}
...
static INT32 MT_ATESetTxAntenna(RTMP_ADAPTER *pAd, UINT32 Ant)
{
	...
	if (IS_MT7615(pAd)) {
		...
		if (ATECtrl->TxAntennaSel & 0x1) {
			...
			HW_IO_READ32(pAd, CR_WF3_RFINTF_00, &Value);
			Value &= ~CR_RFINTF_ENABLE_ANT_MASK;
			Value |= CR_RFINTF_ENABLE_ANT(0xA);
			HW_IO_WRITE32(pAd, CR_WF3_RFINTF_00, Value);
		}

		if (ATECtrl->TxAntennaSel & 0x2) {
			HW_IO_READ32(pAd, CR_WF3_RFINTF_01, &Value);
			Value &= ~CR_RFINTF_ENABLE_ANT_MASK;
			Value |= CR_RFINTF_ENABLE_ANT(0x0);
			HW_IO_WRITE32(pAd, CR_WF3_RFINTF_01, Value);
