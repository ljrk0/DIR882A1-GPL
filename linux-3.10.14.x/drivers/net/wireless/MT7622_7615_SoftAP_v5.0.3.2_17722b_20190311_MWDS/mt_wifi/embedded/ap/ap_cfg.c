...
/*TODO: Add patch by Sudenghai 2014-4-30*/
INT	Set_KickAll_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);
/*End by Sudenghai 2014-4-30*/
...
static struct {
	...
	{"assoc",           Show_Assoc_Proc}, //T&W 2013-12-20 linsd add connect status, code from chenqi 
	...
	IN INT cmd)
{
	...
	UCHAR apidx = pObj->ioctl_if;
	PUPLINKAP_INFO pUplinkAPInfo;/*hujian 2017-7-17 add*/
#ifdef WSC_AP_SUPPORT
	UINT WscPinCode = 0;
	...
	case RT_OID_WSC_QUERY_PEER_INFO_ON_RUNNING:
		...
		break;
/*add by hujian 2017-7-17*/
#ifdef APCLI_SUPPORT
		/*WPS连接上之后获取上行ap的参数*/
		case RT_OID_WSC_UPLINK_AP_INFO: 				   
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("#####RT_OID_WSC_UPLINK_AP_INFO"));
            if (pObj->ioctl_if_type == INT_APCLI)/*é?DD?ó?úapcli0*/
            {
                    INT ApCliIdx = pObj->ioctl_if;
                    APCLI_MR_APIDX_SANITY_CHECK(ApCliIdx);
                    pWscControl = &pAd->ApCfg.ApCliTab[ApCliIdx].WscControl;
                    wrq->u.data.length = sizeof(UPLINKAP_INFO);
                    os_alloc_mem(pAd, (UCHAR **)&pUplinkAPInfo, sizeof(UPLINKAP_INFO));
                    if (pUplinkAPInfo == NULL)
                    {
                            Status = -EFAULT;
                            MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("RT_OID_WSC_UPLINK_AP_INFO fail!\n"));
                            break;
                    }

                    RTMPZeroMemory(pUplinkAPInfo, sizeof(UPLINKAP_INFO));
                    pUplinkAPInfo->Channel = pAd->CommonCfg.Channel;/*上行信道*/

                    NdisMoveMemory(&pUplinkAPInfo->WscProfile, &pWscControl->WscProfile, sizeof(WSC_PROFILE));/*?è??profileμ?D??￠·????á11ì??D*/
                    if ((pUplinkAPInfo->WscProfile.Profile[0].AuthType == WSC_AUTHTYPE_OPEN) && (pUplinkAPInfo->WscProfile.Profile[0].EncrType == WSC_ENCRTYPE_NONE))
                    {
                            pUplinkAPInfo->WscProfile.Profile[0].KeyLength = 0;
                            NdisZeroMemory(pUplinkAPInfo->WscProfile.Profile[0].Key, 64);
                    }
                    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("#####pUplinkAPInfo.ProfileCnt=%d\n",pUplinkAPInfo->WscProfile.ProfileCnt));
                    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("#####pUplinkAPInfo.Channel=%d\n",pUplinkAPInfo->Channel));
                    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("#####pUplinkAPInfo.ssid=%s\n",pUplinkAPInfo->WscProfile.Profile[0].SSID.Ssid));
                	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("#####pUplinkAPInfo.AuthType=%d\n",pUplinkAPInfo->WscProfile.Profile[0].AuthType));
            		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("#####pUplinkAPInfo.EncrType=%d,\n",pUplinkAPInfo->WscProfile.Profile[0].EncrType));
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("#####pUplinkAPInfo.Key=%s,\n",pUplinkAPInfo->WscProfile.Profile[0].Key));
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("#####:length:%d,point:%p--\n",wrq->u.data.length,pUplinkAPInfo));

                    if (copy_to_user(wrq->u.data.pointer, pUplinkAPInfo, wrq->u.data.length))
                    {
                            Status = -EFAULT;
                    }
                    os_free_mem(pUplinkAPInfo);
                    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Query::RT_OID_WSC_UPLINK_AP_INFO:status:%d,point:%p--\n",Status,wrq->u.data.pointer));
            }
            else
            {
                    Status = -EFAULT;
            ...
            break;
		/*获取ApCli的连接状态*/
		case RT_OID_WSC_UPLINK_CONN_STATUS: 				   
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("#####RT_OID_WSC_UPLINK_CONN_STATUS"));
            if (pObj->ioctl_if_type == INT_APCLI)
            {
                INT connectStatus = 0;
			 		...
			 		for (i=0; VALID_UCAST_ENTRY_WCID(pAd, i); i++)
			 		{
							...
							&& (tr_entry->PortSecured == WPA_802_1X_PORT_SECURED))
			 			{
							if (pEntry->wdev == &pAd->ApCfg.ApCliTab[ifIndex].wdev) {
								MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ApCli%d         Connected AP : %02X:%02X:%02X:%02X:%02X:%02X   SSID:%s\n",
                ...
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Query::RT_OID_WSC_UPLINK_CONN_STATUS:status:%d,point:%p--\n",connectStatus,wrq->u.data.pointer));
        		wrq->u.data.length = sizeof(INT);
				if (copy_to_user(wrq->u.data.pointer, &connectStatus, wrq->u.data.length))
				{
				        Status = -EFAULT;
				}
			}
...
#endif /* APCLI_SUPPORT */
/*hujian add end*/
/*Add by T&W Sudenghai 2014-11-1 获取接口的rssi值*/
		case RT_OID_GET_LINK_RSSI:
		{
			POS_COOKIE pObj;
			INTERFACE_RSSI_SAMPLE in_rssi_samp;
			UCHAR AntennaNum = 1;
			INT i;
			...
			if ((pObj->ioctl_if_type == INT_APCLI))
			{ 
				memcpy(&in_rssi_samp.RssiSample, &pAd->ApCfg.ApCliTab[pObj->ioctl_if].RssiSample, sizeof(RSSI_SAMPLE));
			}
			else
#endif /* APCLI_SUPPORT */
			{
				memcpy(&in_rssi_samp.RssiSample, &pAd->ApCfg.RssiSample, sizeof(RSSI_SAMPLE));
			}
	
			in_rssi_samp.BbpRssiToDbmDelta = pAd->BbpRssiToDbmDelta;
			in_rssi_samp.TxPath = pAd->Antenna.field.TxPath;
#ifdef APCLI_SUPPORT
			if ((pObj->ioctl_if_type == INT_APCLI))
			{  
				for (i=0; VALID_UCAST_ENTRY_WCID(pAd, i); i++)
				{
					...
					if(NULL != pEntry)
					{
					 // printk("pEntry->wdev->if_dev->name=%s\n",pEntry->wdev->if_dev->name);
					 // printk("0pEntry->Addr=%02x:%02x:%02x:%02x:%02x:%02x\n",pEntry->Addr[0],pEntry->Addr[1],pEntry->Addr[2],pEntry->Addr[3],pEntry->Addr[4],pEntry->Addr[5]);
					} 
					else
						break;
					...
					if (IS_ENTRY_APCLI(pEntry))
					{
						if(0xFF == pEntry->Addr[0])
							continue;
						
						in_rssi_samp.RssiSample.AvgRssi[0] = pEntry->RssiSample.AvgRssi[0];
						in_rssi_samp.RssiSample.AvgRssi[1] = pEntry->RssiSample.AvgRssi[1];
						in_rssi_samp.RssiSample.AvgRssi[2] = pEntry->RssiSample.AvgRssi[2];
						in_rssi_samp.RssiSample.AvgRssi[3] = pEntry->RssiSample.AvgRssi[3];
...
#endif
			if (copy_to_user(wrq->u.data.pointer, &in_rssi_samp, wrq->u.data.length))
			{
				Status = -EFAULT;
			}			
	
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_GET_LINK_RSSI \n")); 
	
			break;
		}
/*End by T&W Sudenghai 2014-11-1*/
...
INT	Set_KickAll_Proc(
	IN	PRTMP_ADAPTER	pAd,
	IN	RTMP_STRING *arg)
{
	UCHAR kick_idx;

	kick_idx = simple_strtol(arg, 0, 10);
	if (kick_idx)
	{
		KickAllSta(pAd);
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==>Set_KickAll_Proc (ON)\n"));
	}
	else
	{
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==>Set_KickAll_Proc (OFF)\n"));
	}
	...
	IN	RTMP_STRING *arg)
{
	...
	if (pAd->ApCfg.AutoChannelAlg == ChannelAlgBusyTime) {
		...
		if ((pObj->ioctl_if_type == INT_MBSSID) || (pObj->ioctl_if_type == INT_MAIN)) {
			...
			pAd->ApCfg.bPartialScanEnable[IfIdx] = TRUE;
			pAd->ApCfg.bPartialScanning[IfIdx] = TRUE;
#endif
#ifdef TR181_SUPPORT
			{
