...
int wlstatus_disconnect_flag_2g = 0;
int wlstatus_disconnect_flag_5g = 0;

EXPORT_SYMBOL(wlstatus_disconnect_flag_2g);
EXPORT_SYMBOL(wlstatus_disconnect_flag_5g);
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	if(pAd->ApCfg.ApCliTab[ifIndex].wdev.channel >= 36)
		wlstatus_disconnect_flag_5g = 1;
	else
		wlstatus_disconnect_flag_2g = 1;
	if ((ifIndex >= MAX_APCLI_NUM)
#ifdef MAC_REPEATER_SUPPORT
		&& (ifIndex < REPT_MLME_START_IDX)
#endif /* MAC_REPEATER_SUPPORT */
