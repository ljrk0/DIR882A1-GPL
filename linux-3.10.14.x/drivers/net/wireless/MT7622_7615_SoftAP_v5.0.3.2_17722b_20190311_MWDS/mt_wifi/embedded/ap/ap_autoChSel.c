	...
	IN struct wifi_dev *wdev)
{
	...
	for (i = 0; i < GroupNum; i++) {
		for (j = 1; j < (GroupNum-i); j++) {
			//modify by taogan for 5G autochannel priority select  band4 > band1
			//if (pSubGroupMaxBusyTimeTable[i] > pSubGroupMaxBusyTimeTable[i+j])
			if ((pSubGroupMaxBusyTimeTable[i] > pSubGroupMaxBusyTimeTable[i+j]) 
				|| ((pSubGroupMaxBusyTimeTable[i] == pSubGroupMaxBusyTimeTable[i+j]) && (TRUE == pAutoChCtrl->AutoChSelCtrl.IsABand)))
			{
	...
	IN struct wifi_dev *pwdev)
{
	...
	pAd->ApCfg.bPartialScanEnable[0] = TRUE;
	pAd->ApCfg.bPartialScanning[0] = TRUE;
#endif
