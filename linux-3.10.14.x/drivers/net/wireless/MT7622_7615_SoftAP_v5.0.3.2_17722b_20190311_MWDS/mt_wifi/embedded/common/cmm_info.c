...
extern _k_ARC_ZeroWait_DFS_CAC_Time_Meet_report_callback_fun_type DfsCacTimeOutCallBack;
...
INT	rtmp_set_channel(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, UCHAR Channel)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		if (Success == TRUE) {
			...
			if (HcIsRfSupport(pAd, RFIC_5GHZ) && (pAd->CommonCfg.bIEEE80211H == 1)) {
				INT IdBss = 0;
				BOOLEAN BandInCac[DBDC_BAND_NUM];
				UCHAR i;
				BSS_STRUCT *pMbssID = NULL;
				...
				for (IdBss = 0; IdBss < pAd->ApCfg.BssidNum; IdBss++) {
					pMbssID = &pAd->ApCfg.MBSSID[IdBss];
					wdev_bss = &pMbssID->wdev;

					if ((pMbssID == NULL) || (wdev_bss == NULL))
						continue;
					if (wdev_bss->pHObj == NULL)
						continue;

					BandIdx = HcGetBandByWdev(wdev_bss);
					...
					if (pDot11hTest->RDMode == RD_SILENCE_MODE) {
						...
						if (DfsCacTimeOutCallBack) {
							DfsCacTimeOutCallBack(RDD_BAND0, pAd->CommonCfg.DfsParameter.Bw, pAd->CommonCfg.DfsParameter.Band0Ch);
						}
...
/*
    ...
    ==========================================================================
*/
INT	Set_DfsEnable_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
    LONG DfsEnable;

	DfsEnable = simple_strtol(arg, 0, 10);

	if (DfsEnable == 1)
		pAd->CommonCfg.DfsParameter.bDfsEnable = TRUE;
	else if (DfsEnable == 0)
		pAd->CommonCfg.DfsParameter.bDfsEnable = FALSE;
	else
		return FALSE;  /*Invalid argument */

	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_DfsEnable_Proc::(IEEE80211H=%d)\n", pAd->CommonCfg.DfsParameter.bDfsEnable));
	...
	IN	RTMP_IOCTL_INPUT_STRUCT	 *wrq)
{
	...
	BSS_ENTRY *pBss;
	UINT32 TotalLen, BufLen = IW_SCAN_MAX_DATA*3;
#ifdef WSC_INCLUDED
...
max_len += OWETRANSIE_LINE_LEN;
	...
	while ((ScanRunning(pAdapter) == TRUE) && (WaitCnt++ < 30))
	{
		OS_WAIT(500);
	}
	if(WaitCnt >= 28)
	{
		printk("MT7615-------->ApSyncMachine.CurrState=%d\n",pAdapter->Mlme.ApSyncMachine.CurrState);
		pAdapter->Mlme.ApSyncMachine.CurrState = AP_SYNC_IDLE;
	...
	sprintf(msg + strlen(msg), "\n");
		...
		if (pBss->bSupportMWDS)
			sprintf(msg + strlen(msg), "%-4s", " YES");
		else
			sprintf(msg + strlen(msg), "%-4s", " NO");
...
#define N_SS_Len			3
	...
	if (wrq->u.data.length == 0  || wrq->u.data.length == 1)
		BufLen = IW_SCAN_MAX_DATA*3 - ChannelInfoLen;
	else
...
//T&W 2019-10-14  add connect status, code from zegnqinghuang 
int wlconfig_assoc_flag_2g = 0;
int wlconfig_assoc_flag_5g = 0;

EXPORT_SYMBOL(wlconfig_assoc_flag_2g);
EXPORT_SYMBOL(wlconfig_assoc_flag_5g);

INT	Show_Assoc_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	INT i;
	INT flag_2g = 0, flag_5g = 0;
	POS_COOKIE pObj;
 	UCHAR ifIndex;
 	...
 	for (i=0; VALID_UCAST_ENTRY_WCID(pAd, i); i++)
	{
		...
		{
			if((WPA_802_1X_PORT_SECURED == tr_entry->PortSecured))//pEntry->wdev->if_dev->name
			{
				if (pEntry->wdev == &pAd->ApCfg.ApCliTab[ifIndex].wdev) {
					if(pEntry->wdev->channel >= 36)
						flag_5g = 1;
					else
						flag_2g = 1;
				}	
	...
	if(1 == flag_2g)
		wlconfig_assoc_flag_2g = 1;
	else
		wlconfig_assoc_flag_2g = 0;

	if(1 == flag_5g)
		wlconfig_assoc_flag_5g = 1;
	else
		wlconfig_assoc_flag_5g = 0;
