	...
	IN  PWSC_CTRL           pWscControl)
{
	...
	//printk("WscProcessCredential()===========================in\n");
		...
		switch (WscType)
		{
			...
			case WSC_ID_SSID:
				...
				if (RTMPCheckStrPrintAble((CHAR *)pData, Idx) || (pWscControl->bCheckMultiByte == FALSE))
					...
					return FALSE;
				//printk("WscProcessCredential()===========================pProfile->Profile[CurrentIdx].SSID.Ssid=%s\n",pProfile->Profile[CurrentIdx].SSID.Ssid);
				break;
			...
			case WSC_ID_AUTH_TYPE:
                ...
                //printk("WscProcessCredential()===========================pProfile->Profile[CurrentIdx].AuthType=%d\n",pProfile->Profile[CurrentIdx].AuthType);
			...
			case WSC_ID_ENCR_TYPE:
				...
				//printk("WscProcessCredential()===========================pProfile->Profile[CurrentIdx].EncrType=%d\n",pProfile->Profile[CurrentIdx].EncrType);
	...
	printk("WscProcessCredential()===========================AuthType=%d\n",pProfile->Profile[CurrentIdx].AuthType);
	printk("WscProcessCredential()===========================EncrType=%d\n",pProfile->Profile[CurrentIdx].EncrType);	
			
	if (((pProfile->Profile[CurrentIdx].EncrType == WSC_ENCRTYPE_TKIP) && (pProfile->Profile[CurrentIdx].AuthType== WSC_AUTHTYPE_WPAPSK))
				|| ((pProfile->Profile[CurrentIdx].EncrType == WSC_ENCRTYPE_TKIP) && (pProfile->Profile[CurrentIdx].AuthType== WSC_AUTHTYPE_WPA2PSK)))

			{
			    pProfile->Profile[CurrentIdx].AuthType = (WSC_AUTHTYPE_WPAPSK | WSC_AUTHTYPE_WPA2PSK);
				pProfile->Profile[CurrentIdx].EncrType = (WSC_ENCRTYPE_TKIP | WSC_ENCRTYPE_AES) ;
...
#ifdef WSC_V2_SUPPORT
	//printk("WscProcessCredential()===========================bEnableWpsV2=%d\n",pWscControl->WscV2Info.bEnableWpsV2);
	...
	if(pWscControl->WscV2Info.bEnableWpsV2)
	{
		...
		*/
		for (Idx = 0; Idx < pProfile->ProfileCnt; Idx++)
		{
			...
			//printk("WscProcessCredential()===========================pCredential->AuthType=%d\n",pCredential->AuthType);
			//printk("WscProcessCredential()===========================pCredential->EncrType=%d\n",pCredential->EncrType);	
...
#ifdef WIFI_CER_SUPPORT//defined(PRODUCT_DIR853_A1) || defined(PRODUCT_DIR853_A2)					
			if (((pCredential->EncrType == WSC_ENCRTYPE_TKIP) && (pCredential->AuthType== WSC_AUTHTYPE_WPAPSK))
				|| ((pCredential->EncrType == WSC_ENCRTYPE_TKIP) && (pCredential->AuthType== WSC_AUTHTYPE_WPA2PSK)))
	...
	//printk("WscProcessCredential()===========================bReject=%d\n",bReject);	
	...
	OUT	VOID *pbuf)
{
    ...
    //printk("________________%s():%d________in\n",__FUNCTION__,__LINE__);
	...
	OUT	VOID *pbuf)
{
	...
	//printk("BuildMessageM2()--------------------->WscConfigMethods=0x%x\n",ConfigMethods);
#endif
	ConfigMethods = cpu2be16(ConfigMethods);
	templen = AppendWSCTLV(WSC_ID_CONFIG_METHODS, pData, (UINT8 *)&ConfigMethods, 0);
	pData += templen;
	...
	OUT	VOID *pbuf)
{
		...
		(pWscControl->EntryIfIdx < MIN_NET_DEVICE_FOR_APCLI))
    {
		...
		*/
		//printk("BuildMessageM7()==============pWscControl->RegData.PeerInfo.Version2==========%d\n",pWscControl->RegData.PeerInfo.Version2);
		if (pWscControl->RegData.PeerInfo.Version2 == 0)
		{
			if (authType == (WSC_AUTHTYPE_WPAPSK | WSC_AUTHTYPE_WPA2PSK))
			{
				...
				//printk("========WSC_AUTHTYPE_WPA2PSK=====\n");
			}
			if (encyType == (WSC_ENCRTYPE_TKIP | WSC_ENCRTYPE_AES))
			{
				encyType = WSC_ENCRTYPE_AES;
				//printk("==============WSC_ENCRTYPE_AES==========\n");
			}
		}
		//printk("BuildMessageM7()==============authType==========%d\n",authType);
		//printk("BuildMessageM7()==============encyType==========%d\n",encyType);
		...
		printk("Ssid:%s,MacAddr:%s,key:%s\n", pCredential->SSID.Ssid,pCredential->MacAddr,pCredential->Key);
	}
	...
	IN  PWSC_CTRL       pWscControl)
{
	...
	if(NULL == IV_DecrData)
	{
		printk("=============WSC_ERROR_CAN_NOT_ALLOCMEM============\n");
		ret = WSC_ERROR_CAN_NOT_ALLOCMEM;
		return ret;
	}
	/* Copy the content to Regdata for lastRx information */
	...
	while (Length > 4)
	{
		...
		switch (WscType)
		{
			...
			case WSC_ID_ENCR_SETTINGS:
				...
				printk("===============ProcessMessageM8()===========call  WscProcessCredential()\n");
