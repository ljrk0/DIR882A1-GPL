...
static INT multi_profile_read(CHAR *fname,CHAR *buf)
{
	...
	while(srcf.Status && (CntSleep < 50))//×î¶à¶Á25´Î¼´10ÃëÖÓ
	{
		/* Sleep 200 milliseconds so pending io might finish normally */
		RtmpusecDelay(200*1000);
		srcf = os_file_open(fname,O_RDONLY,0);
		CntSleep = CntSleep + 1;
		printk("multi_profile_read() Open file \"%s\" failed CntSleep=%d!\n", fname,CntSleep);
	}
#endif	
	if (srcf.Status)
	{
	...
	{
        printk("multi_profile_read() Open file \"%s\" OK CntSleep=%d!\n", fname,CntSleep);
	}
