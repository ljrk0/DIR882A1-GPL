	...
	IN  BOOLEAN			bOff)
{
	...
	else
	{
		...
		if (pAd->ApCfg.MBSSID[ApIdx & 0x0F].WscControl.WscConfMode != WSC_DISABLE)
		{
			...
			MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WscOnOff - ON.\n"));
		}
	}
	MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WscOnOff - bWpsEnable = %d\n", pWpsV2Info->bWpsEnable));
}
	...
	IN  PUCHAR			pMacAddr)
{
    ...
    {
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_WARN, ("The AccessControlList is full, and no more entry can join the list!\n"));		
		return;
	}
