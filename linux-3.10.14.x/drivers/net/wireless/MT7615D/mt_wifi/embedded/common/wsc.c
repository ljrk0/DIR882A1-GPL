...
/* T&W vvv */
void ReportWpsStatusToUsr(int Event_flag)
{
       proc_wsc_status = Event_flag;
}
/* T&W ^^^ */
...
#define WSC_UPNP_TIMER_PENDIND_WAIT	2000
	...
	else
	{
		...
		if (pWscData != NULL)
		{
			...
			//printk("________________%s():%d________BuildMessageNACK\n",__FUNCTION__,__LINE__);
    ...
    IN MLME_QUEUE_ELEM  *Elem)
{
	...
	//printk("WscEAPOLStartAction=========================>pWpsCtrl->WscActionMode:%d\n",pWpsCtrl->WscActionMode);
	//printk("WscEAPOLStartAction=========================>pWpsCtrl->bWscTrigger:%d\n",pWpsCtrl->bWscTrigger);
	...
	*/
	if (pWpsCtrl->WscPeerList.size != 0)
	{
		...
		{
			MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF, ("This is not first WSC peer, ignore this EAPOL_Start!\n"));
	...
	{
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF, ("WscEAPOLStartAction - receive EAPOL-Start from %02x:%02x:%02x:%02x:%02x:%02x\n",
                                 PRINT_MAC(pEntry->Addr)));
	}
#if 0
	if((0 == pWpsCtrl->WscActionMode) && (0 == pWpsCtrl->bWscTrigger))
		{
			printk("WscEAPOLStartAction=========================>wps IR not trggle,return!\n");
			WscSendEapFail(pAd, pWpsCtrl, TRUE);
			return;
		}
	
	if((4 == pWpsCtrl->WscActionMode) && (0 == pWpsCtrl->bWscTrigger))
		{
			printk("WscEAPOLStartAction=========================>wps IR not trggle,return!\n");
			WscSendEapFail(pAd, pWpsCtrl, TRUE);
			return;
		...
		(pWpsCtrl->WscState <= WSC_STATE_WAIT_REQ_ID))
    {
        ...
        MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF, ("Ignore EAPOL-Start.\n"));
	//printk("WscEAPOLStartAction==========end===============>pWpsCtrl->WscActionMode:%d\n",pWpsCtrl->WscActionMode);
	//printk("WscEAPOLStartAction==========end===============>pWpsCtrl->bWscTrigger:%d\n",pWpsCtrl->bWscTrigger);
	...
	IN	MLME_QUEUE_ELEM	*Elem)
{
...
#if 0//def WIFI_CER_SUPPORT//defined(PRODUCT_DIR853_A1) || defined(PRODUCT_DIR853_A2)


	//(pWscControl->WscActionMode == WSC_ENROLLEE)


	printk("WscEAPAction=========================>MsgType:%x\n",MsgType);
	

	if(WSC_MSG_EAP_RSP_ID == MsgType)
		printk("WscEAPAction=========================>MsgType:%s\n","WSC_MSG_EAP_RSP_ID");
	if(WSC_MSG_EAP_REG_RSP_ID == MsgType)
		printk("WscEAPAction=========================>MsgType:%s\n","WSC_MSG_EAP_REG_RSP_ID");
	if(WSC_MSG_EAP_ENR_RSP_ID == MsgType)
		printk("WscEAPAction=========================>MsgType:%s\n","WSC_MSG_EAP_ENR_RSP_ID");
	if(WSC_MSG_EAP_UPNP_RSP_ID == MsgType)
		printk("WscEAPAction=========================>MsgType:%s\n","WSC_MSG_EAP_UPNP_RSP_ID");
	if(WSC_MSG_EAP_REQ_ID == MsgType)
		printk("WscEAPAction=========================>MsgType:%s\n","WSC_MSG_EAP_REQ_ID");
	if(WSC_MSG_EAP_REQ_START == MsgType)
		printk("WscEAPAction=========================>MsgType:%s\n","WSC_MSG_EAP_REQ_START");
	if(WSC_MSG_EAP_FAIL == MsgType)
		printk("WscEAPAction=========================>MsgType:%s\n","WSC_MSG_EAP_FAIL");
	if(WSC_MSG_EAP_FRAG_ACK == MsgType)
		printk("WscEAPAction=========================>MsgType:%s\n","WSC_MSG_EAP_FRAG_ACK");
	if(WSC_MSG_PROB_RSP == MsgType)
		printk("WscEAPAction=========================>MsgType:%s\n","WSC_MSG_PROB_RSP");
	if(WSC_MSG_EAPOL_START == MsgType)
		printk("WscEAPAction=========================>MsgType:%s\n","WSC_MSG_EAPOL_START");
	if(WSC_MSG_M1 == MsgType)
		printk("WscEAPAction=========================>MsgType:%s\n","WSC_MSG_M1");
	if(WSC_MSG_M2 == MsgType)
		printk("WscEAPAction=========================>MsgType:%s\n","WSC_MSG_M2");
	if(WSC_MSG_M2D == MsgType)
		printk("WscEAPAction=========================>MsgType:%s\n","WSC_MSG_M2D");
	if(WSC_MSG_M3 == MsgType)
		printk("WscEAPAction=========================>MsgType:%s\n","WSC_MSG_M3");
	if(WSC_MSG_M4 == MsgType)
		printk("WscEAPAction=========================>MsgType:%s\n","WSC_MSG_M4");
	if(WSC_MSG_M5 == MsgType)
		printk("WscEAPAction=========================>MsgType:%s\n","WSC_MSG_M5");
	if(WSC_MSG_M6 == MsgType)
		printk("WscEAPAction=========================>MsgType:%s\n","WSC_MSG_M6");
	if(WSC_MSG_M7 == MsgType)
		printk("WscEAPAction=========================>MsgType:%s\n","WSC_MSG_M7");
	if(WSC_MSG_M8 == MsgType)
		printk("WscEAPAction=========================>MsgType:%s\n","WSC_MSG_M8");
	if(WSC_MSG_WSC_ACK == MsgType)
		printk("WscEAPAction=========================>MsgType:%s\n","WSC_MSG_WSC_ACK");
	if(WSC_MSG_WSC_DONE == MsgType)
		printk("WscEAPAction=========================>MsgType:%s\n","WSC_MSG_WSC_DONE");
	if(WSC_MSG_UNKNOWN == MsgType)
		printk("WscEAPAction=========================>MsgType:%s\n","WSC_MSG_UNKNOWN");
#endif
	...
	//printk("WscEAPAction=========================>pWscControl->WscActionMode:%d\n",pWscControl->WscActionMode);
	//printk("WscEAPAction=========================>pWscControl->bWscTrigger:%d\n",pWscControl->bWscTrigger);
	...
	else if (MsgType == WSC_MSG_EAP_ENR_RSP_ID)
	{
		...
		else
		{
				...
				(pAdapter->CommonCfg.WscPBCOverlap == TRUE))
			{
				...
				/* RaiderLu 20161102 Success */
				ReportWpsStatusToUsr(STATUS_WSC_USER_OVERLAP);
				/* RaiderLu 20161102 Success */
			}
			else
#endif /* CONFIG_AP_SUPPORT */
	...
	else if (MsgType == WSC_MSG_WSC_NACK)
	{
		...
		//printk("________________%s():%d________bUPnPMsg=%d\n",__FUNCTION__,__LINE__,bUPnPMsg);
		//printk("________________%s():%d________WscState=%d\n",__FUNCTION__,__LINE__,pWscControl->WscState);
		if (bUPnPMsg)
		{
		    //printk("________________%s():%d________bUPnPMsg=%d\n",__FUNCTION__,__LINE__,bUPnPMsg);
			...
			(WscCheckNonce(pAdapter, Elem, FALSE, pWscControl) || WscCheckNonce(pAdapter, Elem, TRUE, pWscControl)))
		{
			...
			*/
			...
			//printk("________________%s():%d________bUPnPMsg=%d  pEntry->Addr=%02x:%02x:%02x:%02x:%02x:%02x\n",__FUNCTION__,__LINE__,bUPnPMsg,pEntry->Addr[0],pEntry->Addr[1],pEntry->Addr[2],pEntry->Addr[3],pEntry->Addr[4],pEntry->Addr[5]);
                //return;
			...
			//printk("________________%s():%d________CurOpMode=%d\n",__FUNCTION__,__LINE__,CurOpMode);
			//printk("________________%s():%d________bReSetWscIE=%d\n",__FUNCTION__,__LINE__,bReSetWscIE);
	...
	{
	//printk("________________%s():%d________\n",__FUNCTION__,__LINE__);
	...
	IN  PWSC_CTRL       pWscControl)
{
...
#if 0
		printk("WscEapEnrolleeAction=========================>MsgType:%x\n",MsgType);
	

	if(WSC_MSG_EAP_RSP_ID == MsgType)
		printk("WscEapEnrolleeAction=========================>MsgType:%s\n","WSC_MSG_EAP_RSP_ID");
	if(WSC_MSG_EAP_REG_RSP_ID == MsgType)
		printk("WscEapEnrolleeAction=========================>MsgType:%s\n","WSC_MSG_EAP_REG_RSP_ID");
	if(WSC_MSG_EAP_ENR_RSP_ID == MsgType)
		printk("WscEapEnrolleeAction=========================>MsgType:%s\n","WSC_MSG_EAP_ENR_RSP_ID");
...
#endif	
	printk("WscEapEnrolleeAction=========================>pWscControl->WscState:%d\n",pWscControl->WscState);
	...
	switch (MsgType)
	{
		...
		case WSC_MSG_EAP_REQ_START:
			...
			//printk("WscEapEnrolleeAction(line%d) pWscControl->WscState=%d\n",__LINE__,pWscControl->WscState);
			/* Change the state to next one */
			if (pWscControl->WscState < WSC_STATE_SENT_M1)
		        pWscControl->WscState = WSC_STATE_SENT_M1;
#ifdef WIFI_CER_SUPPORT//defined(PRODUCT_DIR853_A1) || defined(PRODUCT_DIR853_A2)			
			pWscControl->WscState = WSC_STATE_SENT_M1;
#endif
...
Fail:
    ...
    if (rv)
    {
		...
		else if ((rv == WSC_ERROR_HASH_FAIL) || (rv == WSC_ERROR_HMAC_FAIL))
		{
			...
			//printk("==========2 %s======WSC_ERROR_DECRYPT_CRC_FAIL==========\n",__FUNCTION__);
		}
	...
	IN  PWSC_CTRL       pWscControl)
{
	...
	NdisZeroMemory(WscData, MaxWscDataLen);
	//printk("WscEapApProxyAction=========================>MsgType:%x\n",MsgType);
			
#if 0		
			if(WSC_MSG_EAP_RSP_ID == MsgType)
				printk("WscEapApProxyAction=========================>MsgType:%s\n","WSC_MSG_EAP_RSP_ID");
			if(WSC_MSG_EAP_REG_RSP_ID == MsgType)
				printk("WscEapApProxyAction=========================>MsgType:%s\n","WSC_MSG_EAP_REG_RSP_ID");
			if(WSC_MSG_EAP_ENR_RSP_ID == MsgType)
				printk("WscEapApProxyAction=========================>MsgType:%s\n","WSC_MSG_EAP_ENR_RSP_ID");
	...
	IN  PWSC_CTRL       pWscControl)
{
	...
	MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WscEapRegistrarAction Enter!\n"));
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAdapter)
			CurOpMode = AP_MODE;
#endif // CONFIG_AP_SUPPORT //
    //printk("WscEapRegistrarAction() MsgType=%d\n",MsgType);
	...
	NdisZeroMemory(WscData, MaxWscDataLen);
	//printk("WscEapRegistrarAction=========================>MsgType:%x\n",MsgType);
		
#if 0	
		if(WSC_MSG_EAP_RSP_ID == MsgType)
			printk("WscEapRegistrarAction=========================>MsgType:%s\n","WSC_MSG_EAP_RSP_ID");
		if(WSC_MSG_EAP_REG_RSP_ID == MsgType)
			printk("WscEapRegistrarAction=========================>MsgType:%s\n","WSC_MSG_EAP_REG_RSP_ID");
		if(WSC_MSG_EAP_ENR_RSP_ID == MsgType)
			printk("WscEapRegistrarAction=========================>MsgType:%s\n","WSC_MSG_EAP_ENR_RSP_ID");
	...
	switch (MsgType)
	{
		...
		case WSC_MSG_WSC_DONE:
			...
			if (pWscControl->WscState == WSC_STATE_WAIT_DONE)
			{
				...
				/* RaiderLu 20161102 Success */
				ReportWpsStatusToUsr(STATUS_WSC_USER_SUCCESS);
				/* RaiderLu */
				pWscControl->WscState = WSC_STATE_CONFIGURED;
				RTMPSendWirelessEvent(pAdapter, IW_WSC_STATUS_SUCCESS, NULL, (pWscControl->EntryIfIdx & 0x0F), 0);
#ifdef VENDOR_FEATURE6_SUPPORT
...
Fail:
    ...
    if (rv)
    {
    	if (rv <= WSC_ERROR_DEV_PWD_AUTH_FAIL)
    	{
			...
			{
				pWscControl->RegData.SelfInfo.ConfigError = WSC_ERROR_DECRYPT_CRC_FAIL;
				//printk("=============%s==1 WSC_ERROR_DECRYPT_CRC_FAIL=========\n",__FUNCTION__);
			}
    ...
    IN PVOID SystemSpecific3)
{
	...
	/* TW  Success */
	ReportWpsStatusToUsr(STATUS_WSC_USER_TIMEOUT);
	/* TW */
	MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("<----- Wsc2MinsTimeOutAction\n"));
}
	...
	if (CurOpMode == AP_MODE)
	{
		if (!RTMP_TEST_FLAG(pWscControl, 0x04)) {
			...
			if (WMODE_CAP_2G(wdev->PhyMode))
			    sprintf(&pWscControl->RegData.SelfInfo.DeviceName[0], "%s_%s", AP_WSC_DEVICE_NAME, "2G");
			else if (WMODE_CAP_5G(wdev->PhyMode))
			    sprintf(&pWscControl->RegData.SelfInfo.DeviceName[0], "%s_%s", AP_WSC_DEVICE_NAME, "5G");
			else
...
#define WSC_NVRAM_APPNAME  "nvram_daemon"
	...
	OUT PWSC_PROFILE        pWscProfile)
{
	...
	if (CurOpMode == AP_MODE)
	{
		if ((OpMode & 0x0F) == AP_MODE)
		{
			...
			*/
			...
			{
			    //printk("WscCreateProfileFromCfg()=================>WscSecurityMode(%d):%d\n",apidx,pAd->ApCfg.MBSSID[apidx].WscSecurityMode);
	...
	//printk("WscCreateProfileFromCfg()=================>authType:%d\n",authType);
	//printk("WscCreateProfileFromCfg()=================>encyType:%d\n",encyType);
			...
			else if (DevicePasswordID == DEV_PASS_ID_PIN)
			{
				...
				*/
					...
					(!pWscCtrl->bWscTrigger || (pWscCtrl->WscMode != WSC_PBC_MODE)))
				{
					RTMP_SEM_LOCK(&pWscCtrl->WscPeerListSemLock);
#ifdef WIFI_CER_SUPPORT//defined(PRODUCT_DIR853_A1) || defined(PRODUCT_DIR853_A2)					
					printk("WscCheckPeerDPID()---------------->add this STA to WscPeerList\n");
					printk("WscCheckPeerDPID()---------------->%02X:%02X:%02X:%02X:%02X:%02X\n",Fr->Hdr.Addr2[0],Fr->Hdr.Addr2[1],Fr->Hdr.Addr2[2],Fr->Hdr.Addr2[3],Fr->Hdr.Addr2[4],Fr->Hdr.Addr2[5]);
					WscClearPeerList(&pWscCtrl->WscPeerList);
#endif
				...
				{
				    //printk("WscCheckPeerDPID()---------------->Do NOT add this STA to WscPeerList\n");
					//printk("WscCheckPeerDPID()---------------->%02X:%02X:%02X:%02X:%02X:%02X\n",Fr->Hdr.Addr2[0],Fr->Hdr.Addr2[1],Fr->Hdr.Addr2[2],Fr->Hdr.Addr2[3],Fr->Hdr.Addr2[4],Fr->Hdr.Addr2[5]);
