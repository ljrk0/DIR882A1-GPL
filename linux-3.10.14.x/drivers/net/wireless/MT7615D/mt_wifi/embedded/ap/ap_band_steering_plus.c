...
#endif
#if 0
	NdisCopyMemory(&inf_status_rsp->NeighborRepInfo, &table->NeighborRepInfo, sizeof(RRM_NEIGHBOR_REP_INFO));
#endif
	BndStrgSendMsg(pAd, &msg);
	BND_STRG_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
	...
	PBND_STRG_CLI_TABLE table)
{
...
#if 0
	{
		...
		UINT8 IEId = 3;
		UINT8 Len = 1;
		UINT8 BssPreference = 0;
		FrameLen = 0;
		RRM_InsertNeighborRepIE(pAd, (Event->u.BTM_REQ_DATA.BTMReq + BTMReqLen), &FrameLen,
		sizeof(RRM_NEIGHBOR_REP_INFO)+3, &table->NeighborRepInfo);
		BTMReqLen += FrameLen;
