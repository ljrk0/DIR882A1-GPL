...
UCHAR* mt7615_get_default_bin_image_from_file(RTMP_ADAPTER *pAd)
{
	RTMP_STRING *src = NULL;
	RTMP_OS_FD srcf;
	INT ret_val;
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("mt7615_get_default_bin_image_from_filen.\n"));
	if (multi_inf_get_idx(pAd) == 0){
		src = "/etc_ro/wlan/MT7615_2G_EEPROM.bin";
	}else if (multi_inf_get_idx(pAd) == 1){
		src = "/etc_ro/wlan/MT7615_5G_EEPROM.bin";
	}else
	{
		goto end;
	}
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("#####src########[%s].\n", src));
	...
	else
	{
		...
		ret_val = RtmpOSFileRead(srcf, (RTMP_STRING *)mt7615_get_default_bin_image(pAd), MAX_EEPROM_BIN_FILE_SIZE);
	...
	return mt7615_get_default_bin_image(pAd);
}
...
static VOID mt7615_chipCap_init(RTMP_ADAPTER *pAd, BOOLEAN b11nOnly, BOOLEAN bThreeAnt)
{
	...
	MT7615_ChipCap.EEPROM_DEFAULT_BIN = mt7615_get_default_bin_image_from_file(pAd);//modify by taogan
	MT7615_ChipCap.EEPROM_DEFAULT_BIN_FILE = mt7615_get_default_bin_image_from_file(pAd);
