	...
	IE_LISTS *ie_lists)
{
    ...
    while (((UCHAR *)eid_ptr + eid_ptr->Len + 1) < ((UCHAR *)Fr + MsgLen))
    {
		...
		case IE_EXT_CAPABILITY:
			...
			{
				if (dlink_hook_chk_client(ie_lists->Addr2, eid_ptr->Octet + 3, eid_ptr->Len))
				{
					ie_lists->vendor_ie.is_dlink_mesh = TRUE;
				}
			}
			/* dlink mesh: end */
#endif /* MWDS */
...
//#endif /* CONFIG_HOTSPOT_R2 */
	...
	*/
...
#if 0
			WPAInstallPairwiseKey(pAd, pEntry->func_tb_idx,
				pEntry, TRUE);
#else
	...
	if (!IS_SECURITY_Entry(pEntry))
		{
		PHEADER_802_11 phdr = (PHEADER_802_11)Elem->Msg;;
