...
SendAuth:
	...
	MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		...
		if (dlink_mesh_ignore_client(pRcvHdr->Addr2))
			{
			pr_info("[MTWF][%s]: ignore Authentication Request from (%pM).\n", __func__, pRcvHdr->Addr2);
			return;
			}
		/* dlink mesh: end */
		...
		&& (auth_info.auth_alg == AUTH_MODE_FT))
	{
		...
		if (pEntry != NULL)
		{
			...
			if (pFtInfoBuf)
			{
				...
				//MTK support fix wpa2 security_vulnerability start  add by zengqinghuang 20171019
				...
				//MTK support fix wpa2 security_vulnerability end  add by zengqinghuang 20171019
			}
		}
		return;
	}
