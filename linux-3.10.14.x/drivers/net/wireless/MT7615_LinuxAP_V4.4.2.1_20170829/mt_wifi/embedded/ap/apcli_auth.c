	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if(IS_ADPTR_EZ_SETUP_ENABLED(pAd))
		EZ_DEBUG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_OFF, ("ApCliMlmeAuthReqAction()\n"));
#endif /* WH_EZ_SETUP */
		...
		if (MAC_ADDR_EQUAL(pAd->ApCfg.ApCliTab[ifIndex].MlmeAux.Bssid, Addr2))
		{
			...
			*pCurrState = APCLI_AUTH_REQ_IDLE;
			...
			if (pAd->ApCfg.ApCliTab[ifIndex].wdev.dlink_mesh_en)
				{
				pr_err("[DLINK_VENDOR_CHECK] ap-client de-auth, reason=%d.\n", Reason);	//remove this after verified
