...
int hwnat_status = 1;
struct proc_dir_entry *proc_dir_hwnat;
static struct proc_dir_entry *entry_hwnat_status;

ssize_t proc_hwnat_status_set(struct file *file, const char __user *buffer, size_t count, loff_t *ppos);

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,6,0))
ssize_t proc_hwnat_status_get(struct file *file, char __user *buffer, size_t count, loff_t *ppos);
static const struct file_operations proc_hwnat_status_fops = {
        .read = proc_hwnat_status_get,
        .write = proc_hwnat_status_set,
};
#else
ssize_t proc_hwnat_status_get(char *page, char **start, off_t off, int count, int *eof, void *data_unused);
#endif
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,6,0))
ssize_t proc_hwnat_status_get(struct file *file, char __user *buffer,
...
#else
ssize_t proc_hwnat_status_get(char *page, char **start, off_t off, int count,
...
ssize_t proc_hwnat_status_set(struct file *file, const char __user *buffer,
						size_t count, loff_t *ppos)
{
...
int hwnat_status_proc_init(void)
{
	if (proc_dir_hwnat == NULL)
		proc_dir_hwnat = proc_mkdir("hw_nat", NULL);

	if(proc_dir_hwnat && entry_hwnat_status == NULL)
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,6,0))
		entry_hwnat_status = proc_create("ipv6_hw_nat", 0x0644, proc_dir_hwnat, &proc_hwnat_status_fops);
#else
		entry_hwnat_status = create_proc_entry("ipv6_hw_nat", 0x0644, proc_dir_hwnat);

	if(entry_hwnat_status)
	{
		entry_hwnat_status->read_proc=(read_proc_t*)&proc_hwnat_status_get;
		entry_hwnat_status->write_proc=(write_proc_t*)&proc_hwnat_status_set;
	}
#endif
