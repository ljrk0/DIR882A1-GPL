...
unsigned long vm_total_pages __read_mostly = 1; /* The total number of pages which the VM controls */
#else
		...
		for_each_evictable_lru(l) {
			...
			*/
			if (((lru_pages >> prio) + 1) >= nr_pages || pass > 3) {
				...
				nr_reclaimed += shrink_list(l, nr_to_scan,lruvec,sc);
...
static void __shrink_page_cache(gfp_t mask)
{
	...
	* is still more than minimally needed. */
	...
	nr_pages = max_t(unsigned long, nr_pages, 8*SWAP_CLUSTER_MAX*20);
	...
	*/
	for (pass = 0; pass < 3; pass++) {
		...
		for (prio = DEF_PRIORITY; prio >= 0; prio--) {
			...
			reclaim_state.reclaimed_slab = 0;
			shrink_slab(&shrink, sc.nr_scanned,global_reclaimable_pages());
